...
/**
 * Options every Windows spawn in this module needs.
 *
 * spawn('cmd.exe', [...]) is not what it looks like. Node escapes arguments on
 * Windows using MSVCRT rules; cmd.exe parses its command line with its own, and
 * the two disagree precisely when the command contains quotes. Node sets this
 * flag automatically when you pass `shell:`, but NOT when you name cmd.exe
 * yourself — which is what this module does.
 *
 * The failure mode is the dangerous one: not an error, but cmd.exe receiving a
 * DIFFERENT command than the operator typed and running it.
 *
 * Verbatim means nothing is escaped for us, so the quoting here is ours to get
 * right — hence the quoted trailing argument paired with /s.
 */
const WIN_SPAWN = Object.freeze({ windowsVerbatimArguments: true })
...
function shellFor (command, platform = PLATFORM()) {
  if (platform === 'win32') {
    return {
      ...
      // Quoted trailing argument + /s: cmd.exe strips exactly the first and last
      // quote and takes everything between verbatim. Without the quotes, a command
      // containing its own quotes (`git commit -m "msg"`) gets re-counted and
      // misparsed.
      args: ['/d', '/s', '/c', `"${command}"`],
      spawnOptions: WIN_SPAWN
    }
  }
  return { cmd: '/bin/bash', args: ['-c', command], spawnOptions: {} }
}
...
function scriptFor (workspaceDir, command, platform = PLATFORM()) {
  ...
  if (platform === 'win32') {
    const p = path.win32.join(workspaceDir, 'task-script.cmd')
    return {
      scriptPath: p,
      ...
      cmd: process.env.ComSpec || 'cmd.exe',
      // Quoted: a workspace under C:\Users\Some Name\ would otherwise split into
      // two arguments and cmd.exe would run the wrong file — or nothing.
      args: ['/d', '/s', '/c', `"${p}"`],
      mode: null,
      spawnOptions: WIN_SPAWN
    }
  ...
  return { scriptPath, content: command, cmd: '/bin/bash', args: [scriptPath], mode: '755', spawnOptions: {} }
}
