...
function bridgeAuth (opts = {}) {
  ...
  return (req, res, next) => {
    ...
    // Fail closed, but say what would open it. A 401 with no remediation sends
    // people to read source to find out a header exists and where its value
    // lives — and browser callers (the Elon sidebar) hit this legitimately the
    // first time, before anyone has pasted a key.
    res.status(401).json({
      error: 'Unauthorized: missing or invalid X-Bridge-Key header',
      // Name only what is true TODAY. Pointing at a command that ships in a
      // later release turns a fixable 401 into a wild goose chase.
      how_to_fix: 'Send header  X-Bridge-Key: <token>  where <token> is the contents of ' + TOKEN_PATH,
      reason: key ? 'key_mismatch' : 'key_missing'
    })
  }
}
