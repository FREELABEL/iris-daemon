...
const CHECKS = [
  ...
  },
  {
    ...
    run (f) {
      // AN ATTACHED MONITOR IS NOT A SECOND EXECUTOR, and the first version of this check
      // said it was. `node index.js` started while the real bridge holds :3200 does not
      // bind: it prints "Bridge already running on :3200 — attaching as monitor" and runs
      // no executor at all — measured, its log has ZERO daemon boot markers (no
      // "Ready — waiting for tasks", no watchdog, no cloud auth, no mounted endpoints).
      //
      // Counting it as an executor made this check cry wolf, and a check that cries wolf
      // gets muted — which is strictly worse than no check, because it also blocks a
      // pipeline. The discriminator is MUTEX OWNERSHIP: a real executor holds :3200
      // (index.js) or ~/.iris/daemon.sock (daemon.js). A monitor holds neither.
      //
      // Those two mutexes are also why there is no third lock to add here: index.js cannot
      // bind a taken port, and daemon.js is single-instance on its socket with a handoff
      // protocol. Both were verified working — a hand-started second index.js attached as
      // a monitor rather than taking over.
      const execs = f.executorPids
      const mons = f.monitorPids || []
      const monNote = mons.length
        ? ` (${mons.length} attached monitor${mons.length > 1 ? 's' : ''}: ${mons.join(', ')} — holding no mutex, running no executor)`
        : ''

      if (execs == null) {
        // Fall back to the old, coarser fact if a caller has not been updated.
        ...
        return pids.length > 1
          ? { status: 'fail', detail: `${pids.length} bridge processes are running (${pids.join(', ')}) and mutex ownership was not measured, so an executor cannot be told from a monitor` }
          : { status: 'pass', detail: `one process (pid ${pids[0]})` }
      }

      if (execs.length === 0) {
        return { status: 'unknown', detail: `no process owns :3200 or the daemon socket — nothing is executing tasks${monNote}` }
      }
      if (execs.length > 1) {
        return {
          status: 'fail',
          detail: `${execs.length} processes own an executor mutex (${execs.join(', ')}). Each claims tasks for this same node id — which races on dispatch and makes each report a capacity that excludes the others' active tasks.${monNote}`
        }
      }
      return { status: 'pass', detail: `one executor (pid ${execs[0]})${monNote}` }
    }
  },
  {
