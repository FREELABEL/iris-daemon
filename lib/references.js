...
const fs = require('fs')
const { requireGit } = require('./require-git')
...
class ReferenceManager {
  ...
  async add (url, options = {}) {
    // Git is not needed to install IRIS or run a node — but indexing a reference
    // REPO does clone it. Say that up front rather than surfacing
    // `Command failed: git clone ...` from three frames down (#184597 shape).
    requireGit('Indexing a reference repository')
